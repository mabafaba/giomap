...
// leafletIO is a global object that is created when the page is loaded, and is used to connect to the server and edit the map.
...
leafletIO = function(map, mapRoom, userID, socket = io({path: "/leafletIO-socket-io"})
){
    leafletIO = {
        map: map,
        ...
        connectMapToServer: function (map, editingLayer) {
            this.socket.emit('joinMapRoom', this.mapRoom);
            this.listenForIncomingEdits(map, editingLayer);
            this.sendOutgoingEdits(map, editingLayer);
            this.addGeometriesFromServerToLayer('/giomap/geojson/'+ this.mapRoom, this.editingLayer);
        ...
        deleteLayerFromServer: function (layer) {
            this.socket.emit('iDeletedAGeometry!', this.LayerToGeoJson(layer));
        },
